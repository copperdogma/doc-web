#!/usr/bin/env python3
"""
Normalize Story 166 Marker JSON output into inspectable doc-web-adjacent surfaces.

This keeps the integration story-local: the script consumes the JSON artifacts
emitted by the Marker lite-api spike, resolves Marker `content-ref` pointers
into per-page HTML, and writes:

- `pages_html.jsonl` in the existing `page_html_v1` schema
- `marker_blocks.jsonl` as an inspectable sidecar preserving Marker block ids,
  bbox data, and section hierarchy for provenance review
- `doc_web_bundle/manifest.json` plus `doc_web_bundle/provenance/blocks.jsonl`
  as a minimal contract-aligned `doc-web` bundle proof
- `runtime_trace.json` with page-level extraction-method / confidence policy data
- `summary.json` with lightweight coverage/provenance signals
"""
...
def _has_structural_content(value: str) -> bool:
    ...
    return bool(_html_to_text(value)) or any(tag in lowered for tag in ("<img", "<table", "<figure", "<li", "<h"))
        ...
        rows.append(
            {
                ...
                "html": block.get("html"),
                "text_chars": len(_html_to_text(block.get("html") or "")),
            }
        )
    return rows
...
def _bundle_document_title(page_rows: list[dict[str, Any]], input_pdf: Path) -> str:
    for row in page_rows:
        ...
        if heading:
            text = _html_to_text(str(heading))
            if text:
                return text
        ...
        runtime_trace.append(
            {
                ...
                "processing_step": tag.get("data-processing-step") if tags else module_id,
...
def normalize_marker_document(
    ...
    *,
    ...
    marker_meta_payload = _read_json(marker_meta) if marker_meta else None
    ...
    for page_index, page_block in enumerate(page_blocks, start=1):
        ...
        llm_request_count = (page_stats.get("block_metadata") or {}).get("llm_request_count", 0)
        block_confidence = 1.0 if extraction_method == "pdftext" and llm_request_count == 0 else None
        processing_step = "marker_lite_api.page_html_v1"
        ...
        page_texts.append(_html_to_text(resolved_html))
    ...
    combined_text = "\n".join(page_texts)
    summary = {
        "schema_version": "story166_marker_page_html_prototype_v2",
        "story_id": 166,
        "module_id": module_id,
        "run_id": run_id,
        "input_artifacts": {
        ...
        "signals": {
            ...
            "nonempty_html_pages": sum(1 for row in page_rows if _html_to_text(row["html"])),
            "token_coverage_vs_pdftotext": _token_coverage(pdftotext_text, combined_text),
        ...
        "provenance_notes": [
            ...
            "doc_web_bundle/provenance/blocks.jsonl now carries contract-aligned block provenance using Marker block ids and bboxes.",
    ...
    return page_rows, block_rows, bundle, runtime_trace, summary
...
def main() -> int:
    args = parse_args()
    document_payload = _read_json(args.marker_json)
    created_at = _utc_now()
    page_rows, block_rows, bundle, runtime_trace, summary = normalize_marker_document(
        document_payload,
    ...
    pages_path = args.outdir / "pages_html.jsonl"
    blocks_path = args.outdir / "marker_blocks.jsonl"
    summary_path = args.outdir / "summary.json"
    runtime_trace_path = args.outdir / "runtime_trace.json"
    bundle_dir = args.outdir / "doc_web_bundle"
    ...
    _write_json(runtime_trace_path, runtime_trace)
    _write_json(bundle_manifest_path, bundle["manifest"])
    _write_jsonl(bundle_provenance_path, bundle["provenance_rows"])
    _write_text(
        bundle_index_path,
        _bundle_index_html(
    ...
    for entry_id, page_html in bundle["page_files"].items():
        _write_text(bundle_dir / f"{entry_id}.html", page_html)
