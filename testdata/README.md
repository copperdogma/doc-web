...
- `mixed-archive-pdf-mini.source.json` / `mixed-archive-pdf-mini.zip`: repo-owned ZIP probe for the first bounded maintained mixed-archive PDF-member continuation. Story 221 uses it to prove `recipe-mixed-archive-zip-routing-mvp.yaml`, `archive_unpack_manifest_v1`, and `archive_route_members_v1` on one four-member archive: flat born-digital PDF, plain-text `.eml`, static HTML snapshot, and one intentionally unsupported `.txt` member. The supported claim is intentionally narrow: ZIP-only entry, archive-relative member manifest/route rows, a nested recommendation-only intake run for the PDF member that stops at an inspectable `intake_plan_v1` artifact, existing maintained direct-entry launches for the `.eml` and HTML members, and explicit blocked outcomes per unsupported member. It is not evidence for direct-folder PDF parity, archive-contained approved handoff or final PDF launch, grouped image-member routing, nested archives, attachments, or broad archive-wide normalization.
