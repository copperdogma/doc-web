...
- `mixed-archive-pdf-mini.source.json` / `mixed-archive-pdf-mini.zip`: repo-owned ZIP probe for the first bounded maintained mixed-archive PDF-member approved-handoff launch continuation. Story 221 establishes the nested recommendation substrate and Story 223 extends the same fixture to prove `recipe-mixed-archive-zip-routing-mvp.yaml`, `archive_unpack_manifest_v1`, and `archive_route_members_v1` on one four-member archive: flat born-digital PDF, plain-text `.eml`, static HTML snapshot, and one intentionally unsupported `.txt` member. The supported claim is intentionally narrow: ZIP-only entry, archive-relative member manifest/route rows, a nested intake run for the PDF member that records an inspectable `intake_plan_v1` artifact, a launched member-local `intake_handoff_v1` artifact, and a launched born-digital PDF child run, existing maintained direct-entry launches for the `.eml` and HTML members, and explicit blocked outcomes per unsupported member. It is not evidence for scanned or handwritten PDF-member launch, grouped image-member routing, nested archives, attachments, or broad archive-wide normalization.
- `mixed-archive-images-mini.source.json` / `mixed-archive-images-mini.zip`: repo-owned ZIP probe for the first bounded maintained mixed-archive grouped image-member OCR-boundary continuation. Story 224 uses it to prove `recipe-mixed-archive-zip-routing-mvp.yaml`, `archive_unpack_manifest_v1`, and `archive_route_members_v1` on one three-member archive: two shared-parent page images under `pages/` plus one intentionally unsupported `.txt` member. The supported claim is intentionally narrow: ZIP-only entry, archive-relative member manifest rows, grouped route rows with shared `group_id` / `group_key` / `group_role` / `group_size` provenance, one grouped `--input-images` launch into `recipe-images-ocr-html-mvp.yaml`, the first downstream `page_image_v1` artifact recorded as `first_downstream_artifact`, the first grouped OCR artifact at `02_ocr_ai_gpt51_v1/pages_html.jsonl` under the shared child run id, and explicit blocked outcomes for unsupported non-image members. It is not evidence for OCR quality improvement beyond that bounded OCR surface, grouped-image continuation beyond `ocr_ai`, nested archives, attachments, or broad archive-wide normalization.
...
- `mixed-folder-pdf-mini.source.json` / `mixed-folder-pdf-mini/`: repo-owned source-native folder probe for the first bounded direct-folder PDF-member approved-handoff launch continuation. Story 222 establishes the direct-folder recommendation substrate and Story 223 uses the same fixture to prove `recipe-mixed-folder-routing-mvp.yaml`, `folder_members_manifest_v1`, and `archive_route_members_v1` on one four-member folder tree: flat born-digital PDF, plain-text `.eml`, static HTML snapshot, and one intentionally unsupported `.txt` member. The supported claim is intentionally narrow: direct-folder entry, relative member manifest/route rows, a nested approved intake run for the PDF member that records an inspectable `intake_plan_v1` artifact, a launched member-local `intake_handoff_v1` artifact, and a launched born-digital PDF child run, existing maintained direct-entry launches for the `.eml` and HTML members, and explicit blocked outcomes per unsupported member. It is not evidence for scanned or handwritten PDF-member launch, grouped image-member routing, nested archives, attachments, or broad folder/archive normalization.
- `mixed-folder-images-mini.source.json` / `mixed-folder-images-mini/`: repo-owned source-native folder probe for the first bounded direct-folder grouped image-member OCR-boundary continuation. Story 224 uses it to prove `recipe-mixed-folder-routing-mvp.yaml`, `folder_members_manifest_v1`, and `archive_route_members_v1` on one three-member folder tree: two shared-parent page images under `pages/` plus one intentionally unsupported `.txt` member. The supported claim is intentionally narrow: direct-folder entry, relative member manifest/route rows, grouped route rows with shared `group_id` / `group_key` / `group_role` / `group_size` provenance, one grouped source-native `--input-images` launch into `recipe-images-ocr-html-mvp.yaml`, the first downstream `page_image_v1` artifact recorded as `first_downstream_artifact`, the first grouped OCR artifact at `02_ocr_ai_gpt51_v1/pages_html.jsonl` under the shared child run id, and explicit blocked outcomes for unsupported non-image members. It is not evidence for OCR quality improvement beyond that bounded OCR surface, grouped-image continuation beyond `ocr_ai`, nested archives, attachments, or broad folder/archive normalization.
...
- Mixed-archive ZIP PDF-member probe fixture:
  ...
  rm -rf "$tmpdir"
  ```
- Mixed-archive ZIP grouped-image probe fixture:
  ```bash
  tmpdir="$(mktemp -d)"
  mkdir -p "$tmpdir/pages" "$tmpdir/notes"
  cp testdata/handwritten-notes-mini-images/page-001.png "$tmpdir/pages/page-001.png"
  cp testdata/handwritten-notes-mini-images/page-002.png "$tmpdir/pages/page-002.png"
  printf '%s\n' 'This unsupported text member exists to prove non-image rows stay explicitly blocked on the bounded grouped-image mixed-archive slice.' > "$tmpdir/notes/readme.txt"
  (
    cd "$tmpdir" &&
    /usr/bin/zip -qr "$OLDPWD/testdata/mixed-archive-images-mini.zip" pages notes
  )
  rm -rf "$tmpdir"
  ```
...
- Mixed-folder PDF-member probe fixture:
  ...
  ```
- Mixed-folder grouped-image probe fixture:
  ```bash
  rm -rf testdata/mixed-folder-images-mini
  mkdir -p testdata/mixed-folder-images-mini/pages testdata/mixed-folder-images-mini/notes
  cp testdata/handwritten-notes-mini-images/page-001.png testdata/mixed-folder-images-mini/pages/page-001.png
  cp testdata/handwritten-notes-mini-images/page-002.png testdata/mixed-folder-images-mini/pages/page-002.png
  printf '%s\n' 'This unsupported text member exists to prove explicit blocked routing on the bounded mixed-folder grouped image-member slice.' > testdata/mixed-folder-images-mini/notes/readme.txt
  ```
