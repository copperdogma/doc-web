...
RESULTS = ROOT / "results/deepseek-v41-20260912"
_REAL_POST = owner.httpx.post


def recorded_post(url, **kwargs):
    body = kwargs["json"]
    body["provider"]["order"] = ["deepinfra/fp8"]
    ...
    reserve = 50000 * 0.0000002 + body["max_tokens"] * 0.0000006
    ...
    if ledger.get("closed"):
        raise RuntimeError("Campaign is closed; use a new approved run identity")
    ...
    (RESULTS / f"request-{seq:03d}.json").write_text(request + "\n")
    record = {
        "sequence": seq,
    ...
    if response.status_code >= 400 and not usage:
        # Missing usage is not proof of zero billing; keep intent unresolved.
        ledger["conservative_unreconciled_exposure_usd"] = reserve
    if (
