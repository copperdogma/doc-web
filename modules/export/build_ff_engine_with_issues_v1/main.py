...
def _dedupe_item_events(events: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for event in events:
        ...
        for idx, existing in enumerate(deduped):
            if existing.get("kind") != "item":
                continue
            if str(existing.get("action") or "").strip().lower() != action:
                continue
...
def make_sequence(portion: Dict[str, Any], section_id: str) -> List[Dict[str, Any]]:
    ...
    # Item events (add/remove/reference)
    item_seen = set()
    ...
    # Item checks (inventory checks + item check fields)
    if inv and isinstance(inv, dict):
    ...
    mechanics_events = _dedupe_item_events(mechanics_events)
