...
def text_quote(value: str) -> Optional[str]:
    collapsed = collapse_text(TAG_RE.sub(" ", value or ""))
    if not collapsed:
        return None
    return collapsed[:400]


def tag_for_element_type(element_type: str, *, first_heading: bool) -> str:
...
def block_kind_for_element_type(element_type: str) -> str:
    if element_type == "Title":
        return "heading"
    if element_type in {"ListItem", "BulletedText"}:
        return "list_item"
...
def bundle_index_html(document_title: str, entries: list[dict[str, Any]]) -> str:
    items = "\n".join(
...
def source_artifact(elements: list[dict[str, Any]], fallback: str) -> str:
    if not elements:
        return fallback
...
def render_table_html(raw_table_html: str, block_id: str) -> str:
    soup = BeautifulSoup(raw_table_html, "html.parser")
    table = soup.find("table")
    if table is None:
        return (
            f'<table id="{html.escape(block_id)}"><tbody><tr><td>'
            f"{html.escape(collapse_text(raw_table_html))}</td></tr></tbody></table>"
        )
    table["id"] = block_id
    return str(table)
...
def render_entry(
    ...
    *,
    ...
    while idx < len(elements):
        ...
        if element_type in {"PageBreak", "Header", "Footer"} and not collapse_text(text):
            idx += 1
            continue
        ...
        if element_type in {"ListItem", "BulletedText"}:
            ...
            while idx < len(elements) and elements[idx].get("type") in {"ListItem", "BulletedText"}:
                ...
                list_items.append(
                    f'<li id="{html.escape(list_block_id)}">{html.escape(collapse_text(list_text))}</li>'
                )
                list_rows.append(
                    provenance_row(
                        module_id=module_id,
        ...
        tag_name = tag_for_element_type(element_type, first_heading=first_heading)
        block_kind = block_kind_for_element_type(element_type)
        rendered_text = text

        if element_type == "Title":
            rendered_html = (
                f'<{tag_name} id="{html.escape(block_id)}">{html.escape(collapse_text(text))}</{tag_name}>'
            )
            first_heading = False
        ...
        if collapse_text(text) or element_type == "Table":
...
def write_bundle(
    *,
    ...
    created_at = utc_now()
    source_path = source_artifact(source_elements, document_title)
    ...
    manifest = DocWebBundleManifest(
        ...
        run_id=run_id,
        created_at=created_at,
        document_id=slugify(document_title),
