...
| `Marker` | End-to-end external comparator for schema-guided extraction and multi-format breadth | Draft benchmark story | [Story 165](/Users/cam/.codex/worktrees/c09a/doc-web/docs/stories/story-165-marker-breadth-comparator-born-digital-pdf.md) | Story 165 narrows the first honest proof to the high-priority `born-digital-pdf` gap using repo-owned `testdata/tbotb-mini.pdf`, rather than repeating the Onward genealogy seam. The thesis is breadth, not provenance-perfect table rescue. Current substrate evidence is still mixed: the only local `arm64` Python is `3.14`, where current `marker-pdf 1.10.2` still fails immediately in Pillow even after a source rebuild; the available Python `3.11` runtimes are `x86_64` only and resolve by backtracking to legacy `marker-pdf 0.2.6` / `surya-ocr 0.4.5`; Docker is reachable but not yet proven as a clean runtime. | Prove a bounded host or container `marker-pdf` runtime that both starts cleanly and emits real benchmark artifacts on `tbotb-mini.pdf`. Widen to other formats only if that first lane is promising and the substrate is real. |
...
5. `Marker` is now the next active bounded comparator via [Story 165](/Users/cam/.codex/worktrees/c09a/doc-web/docs/stories/story-165-marker-breadth-comparator-born-digital-pdf.md), focused on the high-priority `born-digital-pdf` gap rather than the closed Onward hard seam, but it should stay in `Draft` until the runtime ambiguity is resolved.
