...
- **Most credible local use:** selected Marker internals as an upstream substrate for born-digital PDFs where embedded-text structure matters more than genealogy-grade repair.
...
- **Key read:** Story 165 changed the read materially, Story 166 proved the thin seam, and Story 168 now lands an explicit maintained optional born-digital recipe on `testdata/tbotb-mini.pdf`. Stock `Marker` CLI is still a poor fit because it eagerly loads a multi-gigabyte model stack and carries awkward license constraints, but the thinner Marker-internals path now has fresh maintained `driver.py` proof with accepted `doc_web_bundle` / provenance sidecars and the known heading / merged-choice defects repaired.
- **Spend rule:** do not spend more effort on stock `Marker` product adoption. If `Marker` gets more time, spend it on validating and widening the bounded maintained native-text seam from Story 168, or on making its runtime burden more explicit and manageable.
...
| `Marker` | End-to-end comparator first; selected internals as a possible born-digital PDF substrate second | Benchmarked; maintained optional path landed on one fixture | [Story 165](/Users/cam/.codex/worktrees/c09a/doc-web/docs/stories/story-165-marker-breadth-comparator-born-digital-pdf.md), [Story 166](/Users/cam/.codex/worktrees/c09a/doc-web/docs/stories/story-166-marker-internals-born-digital-pdf-substrate.md), [Story 168](/Users/cam/.codex/worktrees/3662/doc-web/docs/stories/story-168-marker-lite-maintained-born-digital-pdf-path.md) | Story 165 closed the broad comparator question, Story 166 proved the `lite_api` seam, and Story 168 now lands a maintained optional native-text recipe that runs through `driver.py` and emits accepted `doc_web_bundle` / provenance proof on `testdata/tbotb-mini.pdf`. Stock `Marker` CLI remains unattractive because of GPL/model-license posture and multi-gigabyte weight loading, but the internal seam now wins the specific born-digital question that matters here: maintained native-text routing plus contract-grade block provenance. It still remains bounded evidence rather than a default-ready path because the runtime is heavy and the fixture breadth is still tiny. | Validate Story 168 and widen beyond the single tiny fixture before any default-routing or broader adoption claim. |
...
5. The broad `Marker` comparator question is now answered enough to stop treating it as an abstract future benchmark. Story 165 says "no" to stock `Marker` adoption; Stories 166 and 168 say "yes" to a bounded maintained optional born-digital substrate, but not yet to default routing or broad format claims.
...
- Follow-up proof path now exists for the next broad external comparator:
  - [Story 165](/Users/cam/.codex/worktrees/c09a/doc-web/docs/stories/story-165-marker-breadth-comparator-born-digital-pdf.md)
