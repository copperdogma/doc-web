...
- **Key read:** Story 165 changed the read materially. Stock `Marker` CLI is still a poor fit because it eagerly loads a multi-gigabyte model stack and carries awkward license constraints, but the thinner Marker-internals path preserved born-digital structure better than the current local OCR-routed baseline on `testdata/tbotb-mini.pdf`.
- **Spend rule:** do not spend more effort on stock `Marker` product adoption. If `Marker` gets more time, spend it only on the thin internal substrate seam now tracked in Story 166.
...
| `Marker` | End-to-end comparator first; selected internals as a possible born-digital PDF substrate second | Benchmarked; bounded spike positive, maintained follow-on named | [Story 165](/Users/cam/.codex/worktrees/c09a/doc-web/docs/stories/story-165-marker-breadth-comparator-born-digital-pdf.md), [Story 166](/Users/cam/.codex/worktrees/c09a/doc-web/docs/stories/story-166-marker-internals-born-digital-pdf-substrate.md), [Story 168](/Users/cam/.codex/worktrees/9f4f/doc-web/docs/stories/story-168-marker-lite-maintained-born-digital-pdf-path.md) | Story 165 closed the broad comparator question enough to sharpen the real seam, and Story 166 now proves that the thinner `lite_api` seam can normalize into a minimal `doc-web` bundle/provenance proof on `testdata/tbotb-mini.pdf` while rebuilding the target runtime from a repo-local cached image instead of a stale worktree-bound container. Stock `Marker` CLI remains unattractive because of GPL/model-license posture and multi-gigabyte weight loading, but the internal seam now clearly beats the current pagelines baseline on the question that matters here most: native-text routing plus accepted `doc-web` block provenance. | Build Story 168: land the maintained born-digital PDF path that reuses the Marker-lite seam, then fix the remaining heading-level and paragraph-splitting defects before any direct runtime adoption claim. |
...
5. The broad `Marker` comparator question is now answered enough to stop treating it as an abstract future benchmark. Story 165 says "no" to stock `Marker` adoption, but "maybe" to a thin Marker-internals born-digital substrate, which is now the narrower follow-on in [Story 166](/Users/cam/.codex/worktrees/c09a/doc-web/docs/stories/story-166-marker-internals-born-digital-pdf-substrate.md).
