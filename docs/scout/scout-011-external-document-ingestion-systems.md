...
| `Marker` | End-to-end comparator first; selected internals as a possible born-digital PDF substrate second | Benchmarked; scoped follow-on candidate | [Story 165](/Users/cam/.codex/worktrees/c09a/doc-web/docs/stories/story-165-marker-breadth-comparator-born-digital-pdf.md), [Story 166](/Users/cam/.codex/worktrees/c09a/doc-web/docs/stories/story-166-marker-internals-born-digital-pdf-substrate.md) | Story 165 closed the broad comparator question enough to sharpen the real seam. Stock `Marker` CLI is too heavy and awkward for adoption here: on this machine it still pulls a multi-gigabyte model stack even for `--disable_ocr`, and the license posture remains unattractive. But the thinner `lite_api` Marker-internals path on `testdata/tbotb-mini.pdf` preserved all tracked headings plus `1.0` token coverage versus `pdftotext`, with `page_stats` and `table_of_contents` metadata, while the current local baseline still routed all pages through `tesseract` despite full `pdftext` coverage. | Build the narrower Story 166 seam: prove whether Marker internals can feed a thin born-digital PDF substrate into `doc-web` without adopting stock Marker CLI or violating the repo's traceability/runtime constraints. |
