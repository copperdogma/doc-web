...
- **C2: Format-Specific Conversion Recipes** (AI capability) — **climb**
  - Current: The maintained intake surface now has two explicit lanes: the recommendation-only recipe still emits `intake_plan_v1` across image-directory and PDF inputs, and the new confirmed-handoff recipe preserves the human approval gate at `confirm_plan_v1` while launching the approved maintained explicit recipe for supported image-directory and PDF plans into a stamped `intake_handoff_v1` artifact. Manual recipe-path retyping is removed for that supported slice, but the broader 10-document workflow replacement is still only partially measured.
  - Target: Auto-detect replaces manual recipe selection for 10 diverse documents.
  - Eval: `auto-book-type-detection` — `accuracy = 1.0`, `overall = 1.0`, `pass_rate = 1.0` on 2026-03-29 via Story 171's refreshed maintained-intake harness over 10 documents; and `approved-intake-handoff` — `pass_rate = 1.0` on 2026-04-01 via Story 176's driver-backed confirmed-handoff proofs on one image-directory input, one scanned PDF input, and one born-digital PDF input. The compromise stays `climb` because the scored 10-document corpus still stops at `intake_plan_v1`, unsupported families still require manual recipe choice/config, and the confirmed-handoff proof surface is representative rather than corpus-wide. Retry when: `new-input-family` or workflow changes that alter the final routing handoff.
