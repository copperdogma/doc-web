---
title: "Improve Real Handwritten Fidelity on the Barney Fixture"
status: "Done"
priority: "High"
ideal_refs:
...
# Story 189 — Improve Real Handwritten Fidelity on the Barney Fixture
...
Story 186 closed the "no real handwriting fixture" gap, but it also established the current product failure boundary: the maintained generic image-directory and PDF OCR lanes stay in the low-0.1 range on the checked-in Barney real handwritten slice, while one bounded stronger direct comparison improved materially to roughly `0.73` without clearing the repo's passing bar. This story should use that now-repo-owned real failure surface to decide and, if justified, land the smallest honest maintained improvement path for Barney-like real handwritten pages. The target is not broad handwritten victory or a hidden special case; it is an evidence-backed rescue decision that either improves real-slice fidelity through the maintained pipeline or records stronger negative evidence and keeps the truth surfaces honest.

## Acceptance Criteria

- [x] A fresh Barney-specific decision surface exists on the current tip that compares:
  - [x] the maintained generic image-directory lane,
  - [x] the maintained generic PDF lane,
  - [x] at least one bounded stronger direct candidate already suggested by Story 186,
  - [x] and any additional candidate needed to make the rescue decision honestly.
...
- [x] If one candidate is materially better and maintainable, the smallest explicit maintained rescue seam lands for the Barney slice on both image-directory and PDF entry paths, with fresh `driver.py` proofs under `output/runs/` and manual artifact inspection showing improved preservation on at least one hard Barney snippet that currently drifts semantically.
- [x] If no candidate justifies a maintained rescue seam, the runtime remains unchanged, but the story records the compared candidates, their measured scores/costs, and the explicit reason no bounded maintained path was adopted.
- [x] `docs/evals/registry.yaml`, `tests/fixtures/formats/_coverage-matrix.json`, `testdata/README.md`, and any related story/truth surfaces move only as far as the fresh evidence justifies. The `handwritten-notes` row does not return to `passing` unless the maintained path genuinely clears the stated bar.
...
- Book-specific deterministic heuristics or hardcoded corrections for Barney names/phrases
...
- **Simplification baseline**: one stronger direct model call already improves the Barney slice substantially over the maintained low-0.1 baseline, so the first move is to measure bounded higher-capability candidates on the checked-in real fixture before adding any rescue logic. If a single model/hint change already solves the slice, that is the right answer.
- **AI-only**: a stronger direct multimodal transcription path may solve Barney-like real handwriting without any downstream heuristics. This is attractive because the repo already owns the source slice and scorer, but it is only worth productizing if the measured quality, cost, and repeatability are clearly better than the current maintained generic seam.
- **Hybrid**: keep the maintained generic OCR lane as the default, then use a bounded stronger rescue only where the real handwritten slice or an explicit quality gate justifies it. This is the leading candidate if all-pages stronger OCR is too costly or if the higher-capability model should stay scoped to real handwritten risk.
...
- [x] Verify Central Tenets:
  - [x] T0 — Traceability: every support claim traces to named Barney fixtures, run IDs, models, and inspected artifacts
  ...
  - [x] T5 — Inspect Artifacts: the actual Barney OCR artifacts are manually opened and checked, not just scored
...
- Either the story lands a bounded handwritten rescue seam with fresh driver-backed evidence on both Barney entry paths, or it proves no such seam is worth maintaining right now.
- In both outcomes, the story leaves behind a repeatable Barney decision surface with artifact paths, candidate results, and honest truth-surface wording.
- No deterministic Barney-specific corrections or silent transcript cleanup are introduced.
