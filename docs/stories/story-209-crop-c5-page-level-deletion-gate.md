...
## 2026-09-06 Qwen3.8 Max 0902 bounded re-evaluation plan (approved)

Exact challenger `qwen/qwen3.8-max-0902`, canonical `qwen/qwen3.8-max-20260902`, Alibaba on OpenRouter pinned without fallback; low reasoning, strict schema, max_tokens=16384. Synthetic qualification max_tokens=1024. Total all-provider cap US$0.75; serial fresh uncached calls, zero automatic retries and at most one explained transport repair. Public owner-identified maintained fixtures plus synthetic square only; provider retention/training uncertainty explicitly approved, no ZDR/data-collection-deny requirement for these payloads.

Preserve prompt/scorer/golden and adapter; a campaign wrapper records complete envelopes and enforces remaining budget before every call. Native synthetic strict image probe, same case adapter parity, Image011 known seal/signature grouping failure, then full13 only if screen passes. Require 13/13, overall>=0.95, zero errors to advance. Current runtime Gemini3Flash stays unchanged; prior maintained 13/13 0.9703 is historical reference, fresh control conditional on competitive result and remaining cap. Current Astra high/medium bounded detector leads do not waive exact runtime safety gates. No implicit LLM judge: maintained task uses deterministic structural scoring plus manual source review. Conditional page-context: qualify separate strict schema, page-122-001 stop first, remaining22 only if passes and affordable; current GPT5.5 22/22 reference is historical. Current owner goldens are authoritative bounded truth; generalization and runtime gates remain distinct. No rollout/default changes, commits or pushes.

Lineage: spec:4, C4/C5, Stories207/209. Same proof surface continuation, no new story. Ideal full-path/runtime proof deferred pending detector gate; no pipeline code changes means driver integration not needed for an access stop or detector rejection. Base b67f001481307ed1d4f2a4194436620dda16528b. Attempt035 reserved.


### 20260906 — Qwen0902 continuation complete

Attempt035 records exact0902/Alibaba strict native and owner-adapter vision qualification, then the predeclared Image011 stop: 0/1, 0.6001, three boxes instead of two; direct source inspection confirms model-wrong seal/signatures splitting. Total $0.013964/$0.75, three calls, no retries or transport errors. Full13, fresh controls and all page-context work remain not measured. Preserve current defaults. Temporary DOC_WEB_OPENROUTER_API_KEY removed by Conductor custody helper.
