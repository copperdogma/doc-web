...
20260318-1600 — **GPT-5.4 Mini/Nano follow-up screen**: Reconstructed missing local eval fixtures from the external Onward scan set, adding `input/onward-to-the-unknown-images-2048/` plus a focused three-page screen task `benchmarks/tasks/onward-single-page-gpt54-screen.yaml` covering `page_024`, `page_030`, and `page_080`. Ran `promptfoo eval -c tasks/onward-single-page-gpt54-screen.yaml --no-cache -j 1`. Results: `GPT-5.4 Mini` averaged **0.235** and `GPT-5.4 Nano` averaged **0.230** on real table pages, both below the earlier `GPT-5 Mini` screen score (**0.272**) and far below the 0.85 viability bar noted above. Failure mode matched the existing budget-model pattern: section-header confusion, row misalignment, and heavy column drift (`32-44` extra output rows on two pages for Mini; Nano also collapsed `BOY/GIRL` structure). Decision reaffirmed: no OpenAI budget-tier adoption for Onward table OCR.
