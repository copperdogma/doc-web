...
Story 189 closed the evidence line honestly: the handwritten family now has two repo-owned real LOC fixtures, and the current bounded Gemini rescue seam still fails both of them. Barney improves materially but remains wrong on names and phrasing, while Alverson fails more fundamentally by stopping after the upper portion of the page and omitting the lower body and postscript. This story is the final handwriting-arc execution attempt: land one generic maintained OCR/escalation path that can read both real LOC fixtures on both image-directory and PDF entry while preserving the synthetic fixtures, or end the arc in this same story with an explicit blocker and unblock condition instead of spawning another same-surface follow-up.

## Acceptance Criteria

- [ ] A fresh current-tip baseline exists for the full five-fixture handwritten corpus (`3` synthetic + `2` real) using the current maintained handwritten recipe surface, and the work log names the exact artifact paths inspected for Barney drift and Alverson lower-page truncation rather than only aggregate ratios.
...
- **Simplification baseline**: already tested and currently negative. Story 189's fresh widened corpus leaves the maintained rescue seam at `pass_rate = 0.6` / `overall_min_ratio = 0.496975`. Fresh direct `gpt-5` probes were worse than the rescue seam (`0.597761` on Barney, `0.310064` on Alverson), and simple 2x upscaling also failed to solve the real fixtures (`0.86197` on Barney, `0.485058` on Alverson). A new story is still warranted because the next honest move is no longer “collect more evidence”; it is “implement one generic full-page rescue or block the line.”
- **AI-only**: still worth checking only if there is genuinely new model evidence. Current repo evidence says a single full-page direct call is not enough for the LOC pair, especially for lower-page continuation text on Alverson.
...
- [ ] Measure the smallest honest generic full-page recovery candidate set before landing new logic:
  - [ ] at least one candidate must target the Alverson lower-page truncation failure directly
...
- [ ] Clear stale `*.pyc`, run through `driver.py` on both real fixtures for both image-directory and PDF entry, verify artifacts in `output/runs/`, and manually inspect the repaired lower-page content
...
- [ ] Verify Central Tenets:
  ...
  - [ ] T3 — Fidelity: lower-page handwritten content is recovered faithfully with no silent omission or normalization
...
The current checked-in LOC handwritten proof pair cannot honestly clear the `0.99` handwritten bar in this repo state because two blockers are now explicit: Barney remains below bar across the maintained rescue seam and the fresh stronger direct-model probes, and the checked-in Alverson front-page source/transcript pair is not source-faithful enough to serve as a clean OCR target for the continuation text Story 189/191 had been treating as “missing lower-page content.”

## Blocker Evidence

- Fresh maintained baseline rerun on current tip wrote `benchmarks/results/handwritten-notes-story191-baseline-20260404.json` with `pass_rate = 0.6`, `overall_min_ratio = 0.499136`, and `page_min_ratio = 0.499136`. The six synthetic rescue cases still passed at `1.0`, but the real fixtures stayed below bar: Barney scored `0.895587` on image entry and `0.756036` on PDF entry, while Alverson scored `0.499136` on image entry and `0.500647` on PDF entry.
- Fresh direct stronger-model probes in this pass did not overturn that result. `gpt-5` direct runs at `output/runs/story191-barney-image-gpt5-direct/02_ocr_ai_gpt51_v1/pages_html.jsonl`, `output/runs/story191-barney-pdf-gpt5-direct/02_ocr_ai_gpt51_v1/pages_html.jsonl`, `output/runs/story191-alverson-image-gpt5-direct/02_ocr_ai_gpt51_v1/pages_html.jsonl`, and `output/runs/story191-alverson-pdf-gpt5-direct/02_ocr_ai_gpt51_v1/pages_html.jsonl` scored `0.192010`, `0.010297`, `0.221929`, and `0.362143`. `claude-opus-4-6` direct image probes at `output/runs/story191-barney-image-claude-direct/02_ocr_ai_gpt51_v1/pages_html.jsonl` and `output/runs/story191-alverson-image-claude-direct/02_ocr_ai_gpt51_v1/pages_html.jsonl` scored `0.725708` and `0.193325`.
- Manual source review changed the interpretation of the Alverson failure. The checked-in source image `testdata/handwritten-notes-alverson-real-images/page-001.jpg` visibly ends on the same line where the current OCR stops (`...exceptable and`). The current checked-in asset also matches the LOC front-page large JPEG dimensions (`638x1024`). Meanwhile the LOC source record for item `2023637835` presents separate `Front` and `Back` images, so the continuation text that the transcript carries after that boundary is not recoverable from the checked-in front-page image alone. This means the current Alverson truth surface is at least partially a source/transcript alignment problem, not just an OCR runtime problem.
- Additional one-off local window and enhancement probes in this pass did not reveal a bounded maintained rescue shape that could rescue the lane honestly. Cropped-window rereads and contrast/upscale variants still failed to recover the missing Alverson continuation from the front-page image and did not demonstrate a reliable Barney improvement strong enough to justify landing new runtime logic.

## Unblock Condition

Unblock this story only when both halves are true:

1. The Alverson truth surface is corrected so the checked-in source assets and transcript actually match the intended OCR target, for example by trimming the transcript to the visible front-page content or by replacing the fixture with a source-faithful front/back pair plus page-separated transcript.
2. A materially different OCR substrate then clears the real pair, not just the synthetic fixtures. That can be a new model, a better source asset, or a genuinely stronger recovery seam, but it must produce fresh current-pass `driver.py` artifacts that bring Barney and the corrected Alverson fixture to `overall_min_ratio >= 0.99`, `page_min_ratio >= 0.99`, and `pass_rate = 1.0`.
...
- **Methodology reality**: this belongs to `spec:1`, `spec:2`, and `spec:8`. Current substrate is `exists` for all three categories in `docs/methodology/state.yaml`, with `C1`, `C2`, and `B5` still active. The relevant coverage row is `handwritten-notes`, which is still `has-fixture` after the widened five-fixture corpus fell to `pass_rate = 0.6`.
