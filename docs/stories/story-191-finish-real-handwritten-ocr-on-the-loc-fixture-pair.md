...
20260721-1800 — Gemini 3.6 Flash / Gemini 3.5 Flash-Lite unblock screen: both exact GA IDs were live-proven, then each ran through `driver.py` on the corrected Barney and Alverson image-entry fixtures. The first scoring pass exposed a scorer defect rather than an OCR collapse: `SequenceMatcher`'s default autojunk heuristic discarded repeated characters in long transcripts and understated near-matches. After disabling autojunk and adding regression coverage, Gemini 3.6 Flash scored `0.984527` Barney / `0.984052` Alverson and Gemini 3.5 Flash-Lite scored `0.974964` / `0.989935`; all four remain below the strict `0.99` bar. Manual inspection of both source scans and all four `pages_html.jsonl` artifacts confirmed genuine remaining literal-transcription substitutions, spelling normalization, and omissions. Artifacts: `output/runs/eval-barney-image-gemini-3-6-flash/02_ocr_ai_gpt51_v1/pages_html.jsonl`, `output/runs/eval-alverson-image-gemini-3-6-flash/02_ocr_ai_gpt51_v1/pages_html.jsonl`, `output/runs/eval-barney-image-gemini-3-5-flash-lite/02_ocr_ai_gpt51_v1/pages_html.jsonl`, and `output/runs/eval-alverson-image-gemini-3-5-flash-lite/02_ocr_ai_gpt51_v1/pages_html.jsonl`. Decision: do not change the maintained rescue path; Story 191 remains `Blocked` until a candidate clears both corrected real fixtures at `0.99`.
20260813-1505 — Gemini 3.7 Flash eval decision contract: screen exact GA ID `gemini-3.7-flash` only on the corrected repo-owned Barney and Alverson image-entry fixtures through `driver.py`, using the existing literal-transcription scorer and a frozen eval-only recipe. The OCR client leaves `thinking_config` unset, which selects Gemini 3.7 Flash's documented medium default; this is appropriate for the quality-sensitive OCR seam, while the crop lanes use explicit low thinking. Promote only if both fixtures reach `overall_ratio >= 0.99`; otherwise keep Story 191 blocked and do not change the maintained rescue recipe. Public LOC fixtures are permitted; repo credentials must remain secret. This is an independent handwriting slot after the crop ladder and remains within the shared `$5` paid-call cap. No runtime default, commit, or push is authorized.
