...
HANDWRITTEN_ALVERSON_REAL_IMAGES_FIXTURE = "testdata/handwritten-notes-alverson-real-images"
...
def test_handwritten_alverson_real_image_recipe_extract_only_smoke(tmp_path):
    run_id = f"image-intake-smoke-{uuid.uuid4().hex[:8]}"
    run_dir = tmp_path / run_id
    ...
    result = subprocess.run(
        [
            ...
            IMAGE_RECIPE,
            "--input-images",
            HANDWRITTEN_ALVERSON_REAL_IMAGES_FIXTURE,
            "--run-id",
            run_id,
            "--output-dir",
            str(run_dir),
    ...
    fixture_abs = str(Path(HANDWRITTEN_ALVERSON_REAL_IMAGES_FIXTURE).resolve())
    assert all(row["source"] == [fixture_abs] for row in rows)
    assert all(Path(row["image"]).exists() for row in rows)
