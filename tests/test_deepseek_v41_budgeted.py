...
SPEC = importlib.util.spec_from_file_location(
    "deepseek_v41_budgeted",
    Path(__file__).parents[1] / "benchmarks/providers/deepseek_v41_budgeted.py",
)
...
def setup_guard(tmp_path, monkeypatch):
    ...
    ledger.write_text(json.dumps({"cap_usd": 0.50, "spent_usd": 0, "calls": []}))
    return ledger
...
def test_bad_cost_blocks_next_call(setup_guard, monkeypatch, cost):
    ...
    provider.recorded_post("fake", json={"max_tokens": 1024, "provider": {}})
...
def test_no_usage_429_keeps_cost_unresolved(setup_guard, monkeypatch):
    class Response:
        status_code = 429
    ...
    monkeypatch.setattr(provider, "_REAL_POST", lambda *a, **k: Response())
    provider.recorded_post("fake", json={"max_tokens": 1024, "provider": {}})
    ledger = json.loads(setup_guard.read_text())
    assert ledger["spent_usd"] == 0
    assert ledger["unresolved_cost"] is True
    assert ledger["conservative_unreconciled_exposure_usd"] == pytest.approx(.0106144)
    assert ledger["calls"][0]["cost_usd"] is None
    with pytest.raises(RuntimeError, match="unresolved"):
        provider.recorded_post("fake", json={"max_tokens": 1024, "provider": {}})


def test_closed_campaign_refuses_replay(setup_guard):
    ledger = json.loads(setup_guard.read_text())
    ledger["closed"] = True
    setup_guard.write_text(json.dumps(ledger))
